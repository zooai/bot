...
ZooBot selects models in this order:
...
- `agents.defaults.models` is the allowlist/catalog of models ZooBot can use (plus aliases).
...
`zoo-bot models` (no subcommand) is a shortcut for `models status`.
...
`zoo-bot models scan` inspects OpenRouter’s **free model catalog** and can
...
agent directory (default `~/.zoo-bot/agents/<agentId>/models.json`). This file
