...
- `/usage cost` in chats: local cost summary aggregated from ZooBot session logs.
- CLI: `zoo-bot status --usage` prints a full per-provider breakdown.
- CLI: `zoo-bot channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
