...
ZooBot can convert outbound replies into audio using ElevenLabs, OpenAI, or Edge TTS.
It works anywhere ZooBot can send audio; Telegram gets a round voice-note bubble.
...
Edge TTS does **not** require an API key. If no API keys are found, ZooBot defaults
...
TTS config lives under `messages.tts` in `zoo-bot.json`.
...
- If `provider` is **unset**, ZooBot prefers `openai` (if key), then `elevenlabs` (if key),
  otherwise `edge`.
...
`~/.zoo-bot/settings/tts.json`, override with `BOT_TTS_PREFS` or
...
- **Edge TTS**: uses `edge.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - If the configured Edge output format fails, ZooBot retries with MP3.
...
Discord note: `/tts` is a built-in Discord command, so ZooBot registers
