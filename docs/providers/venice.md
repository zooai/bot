---
summary: "Use Venice AI privacy-focused models in ZooBot"
read_when:
  - You want privacy-focused inference in ZooBot
...
```bash
zoo-bot onboard --auth-choice venice-api-key
```
...
```bash
zoo-bot agent --model venice/llama-3.3-70b --message "Hello, are you working?"
```

## Model Selection

After setup, ZooBot shows all available Venice models. Pick based on your needs:
...
```bash
zoo-bot models set venice/claude-opus-45
zoo-bot models set venice/llama-3.3-70b
```

List all available models:

```bash
zoo-bot models list | grep venice
```
...
ZooBot automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
zoo-bot agent --model venice/llama-3.3-70b --message "Quick health check"

# Use Claude via Venice (anonymized)
zoo-bot agent --model venice/claude-opus-45 --message "Summarize this task"

# Use uncensored model
zoo-bot agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
zoo-bot agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
zoo-bot agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `zoo-bot models list` to see currently available models. Some models may be temporarily offline.
