...
```bash
zoo-bot onboard --auth-choice openai-api-key
# or non-interactive
zoo-bot onboard --openai-api-key "$OPENAI_API_KEY"
```
...
zoo-bot onboard --auth-choice openai-codex

# Or run OAuth directly
zoo-bot models auth login --provider openai-codex
```
