...
read_when:
  - You want to place an outbound voice call from ZooBot
...
Voice calls for ZooBot via a plugin. Supports outbound notifications and
...
- Use `zoo-bot voicecall ...` or the `voice_call` tool
...
```bash
zoo-bot plugins install ./extensions/voice-call
