...
- Jobs persist under `~/.zoo-bot/cron/` so restarts don’t lose schedules.
...
zoo-bot cron list
zoo-bot cron run <job-id>
zoo-bot cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.zoo-bot/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `zoo-bot cron add/edit` or the cron
...
- CLI equivalents: `zoo-bot cron add --light-context ...` and `zoo-bot cron edit --light-context`.
...
- Job store: `~/.zoo-bot/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.zoo-bot/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned by size and line count).
...
- review growth periodically with `zoo-bot cron runs` and adjust retention before logs become large
...
zoo-bot cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
zoo-bot cron edit <jobId> --agent ops
zoo-bot cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
zoo-bot cron run <jobId>
zoo-bot cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
zoo-bot cron edit <jobId> \
...
```bash
zoo-bot system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`zoo-bot system event`](/cli/system).
