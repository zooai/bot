...
Hooks provide an extensible event-driven system for automating actions in response to agent commands and events. Hooks are automatically discovered from directories and can be managed via CLI commands, similar to how skills work in ZooBot.
...
- **Webhooks**: external HTTP webhooks that let other systems trigger work in ZooBot. See [Webhook Hooks](/automation/webhook) or use `zoo-bot webhooks` for Gmail helper commands.
...
- Extend ZooBot's behavior without modifying core code

## Getting Started

### Bundled Hooks

ZooBot ships with four bundled hooks that are automatically discovered:

- **💾 session-memory**: Saves session context to your agent workspace (default `~/.zoo-bot/workspace/memory/`) when you issue `/new`
- **📎 bootstrap-extra-files**: Injects additional workspace bootstrap files from configured glob/path patterns during `agent:bootstrap`
- **📝 command-logger**: Logs all command events to `~/.zoo-bot/logs/commands.log`
...
During onboarding (`zoo-bot onboard`), you'll be prompted to enable recommended hooks. The wizard automatically discovers eligible hooks and presents them for selection.
...
2. **Managed hooks**: `~/.zoo-bot/hooks/` (user-installed, shared across workspaces)
3. **Bundled hooks**: `<zoo-bot>/dist/hooks/bundled/` (shipped with ZooBot)
...
Hook packs are standard npm packages that export one or more hooks via `zoo-bot.hooks` in
...
Hook packs can ship dependencies; they will be installed under `~/.zoo-bot/hooks/<id>`.
Each `zoo-bot.hooks` entry must stay inside the package directory after symlink
resolution; entries that escape are rejected.

Security note: `zoo-bot hooks install` installs dependencies with `npm install --ignore-scripts`
...
homepage: https://docs.zoo-bot.ai/automation/hooks#my-hook
metadata:
  { "zoo-bot": { "emoji": "🔗", "events": ["command:new"], "requires": { "bins": ["node"] } } }
---
...
The `metadata.zoo-bot` object supports:
...
These hooks are not event-stream listeners; they let plugins synchronously adjust tool results before ZooBot persists them.
...
- **Managed hooks** (`~/.zoo-bot/hooks/`): Shared across workspaces

### 2. Create Directory Structure

```bash
mkdir -p ~/.zoo-bot/hooks/my-hook
cd ~/.zoo-bot/hooks/my-hook
```
...
metadata: { "zoo-bot": { "emoji": "🎯", "events": ["command:new"] } }
---
...
# JSON output
zoo-bot hooks info session-memory --json
```
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.zoo-bot/workspace`)
...
```yaml
metadata: { "zoo-bot": { "events": ["command:new"] } } # Specific
```

Rather than:

```yaml
metadata: { "zoo-bot": { "events": ["command"] } } # General - more overhead
```
...
2. Verify HOOK.md format:
   ...
   ```bash
   cat ~/.zoo-bot/hooks/my-hook/HOOK.md
...
1. Create hook directory:
   ...
   ```bash
   mkdir -p ~/.zoo-bot/hooks/my-hook
   mv ./hooks/handlers/my-handler.ts ~/.zoo-bot/hooks/my-hook/handler.ts
   ```
...
- [Bundled Hooks README](https://github.com/zoo-bot/zoo-bot/tree/main/src/hooks/bundled)
