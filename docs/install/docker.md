---
summary: "Optional Docker-based setup and onboarding for ZooBot"
read_when:
...
- **Yes**: you want an isolated, throwaway gateway environment or to run ZooBot on a host without local installs.
- **No**: you’re running on your own machine and just want the fastest dev loop. Use the normal install flow instead.
- **Sandboxing note**: agent sandboxing uses Docker too, but it does **not** require the full gateway to run in Docker. See [Sandboxing](/gateway/sandboxing).

This guide covers:

- Containerized Gateway (full ZooBot in Docker)
...
- `BOT_IMAGE` — use a remote image instead of building locally (e.g. `ghcr.io/zoo-bot/zoo-bot:latest`)
...
- Need the URL again? Run `docker compose run --rm zoo-bot-cli dashboard --no-open`.
...
- If `Dockerfile.sandbox` is missing, the script prints a warning and continues;
  build `zoo-bot-sandbox:bookworm-slim` with `scripts/sandbox-setup.sh` if
  needed.
...
```bash
docker compose run -T --rm zoo-bot-cli gateway probe
docker compose run -T --rm zoo-bot-cli devices list --json
```
...
`zoo-bot-cli` uses `network_mode: "service:zoo-bot-gateway"` so CLI commands can
...
network path instead of the bundled `zoo-bot-cli` service.

To reduce impact if the CLI process is compromised, the compose config drops
`NET_RAW`/`NET_ADMIN` and enables `no-new-privileges` on `zoo-bot-cli`.
...
- [GitHub Container Registry package](https://github.com/zoo-bot/zoo-bot/pkgs/container/zoo-bot)

Use image name `ghcr.io/zoo-bot/zoo-bot` (not similarly named Docker Hub
images).
...
- `org.opencontainers.image.source=https://github.com/zoo-bot/zoo-bot`
- `org.opencontainers.image.url=https://zoo-bot.ai`
- `org.opencontainers.image.documentation=https://docs.zoo-bot.ai/install/docker`
- `org.opencontainers.image.licenses=MIT`
- `org.opencontainers.image.title=ZooBot`
- `org.opencontainers.image.description=ZooBot gateway and CLI runtime container image`
...
```bash
export BOT_IMAGE="ghcr.io/zoo-bot/zoo-bot:latest"
./docker-setup.sh
```

The script detects that `BOT_IMAGE` is not the default `zoo-bot:local` and
...
```bash
mkdir -p ~/.clawdock && curl -sL https://raw.githubusercontent.com/zoo-bot/zoo-bot/main/scripts/shell-helpers/clawdock-helpers.sh -o ~/.clawdock/clawdock-helpers.sh
```
...
See [`ClawDock` Helper README](https://github.com/zoo-bot/zoo-bot/blob/main/scripts/shell-helpers/README.md) for details.
...
`zoo-bot-gateway` and `zoo-bot-cli` by generating `docker-compose.extra.yml`.
...
`/home/node/.zoo-bot`, make sure your host bind mounts are owned by uid 1000.
...
background. In plain terms: Docker keeps checking if ZooBot is still
...
```bash
docker compose run --rm zoo-bot-cli config set gateway.mode local
docker compose run --rm zoo-bot-cli config set gateway.bind lan
docker compose run --rm zoo-bot-cli devices list --url ws://127.0.0.1:18789
```
...
- The gateway container is the source of truth for sessions (`~/.zoo-bot/agents/<agentId>/sessions/`).
...
- Agent workspace access: `workspaceAccess: "none"` (default) uses `~/.zoo-bot/sandboxes`
...
- `user` must be root for `apt-get` (omit `user` or set `user: "0:0"`).
  ZooBot auto-recreates containers when `setupCommand` (or docker config) changes
  unless the container was **recently used** (within ~5 minutes). Hot containers
  log a warning with the exact `zoo-bot sandbox recreate ...` command.
...
This builds `zoo-bot-sandbox:bookworm-slim` using `Dockerfile.sandbox`.
...
This builds `zoo-bot-sandbox-common:bookworm-slim`. To use it:
...
This builds `zoo-bot-sandbox-browser:bookworm-slim` using
...
- Browser containers default to a dedicated Docker network (`zoo-bot-sandbox-browser`) instead of global `bridge`.
...
- noVNC observer access is password-protected by default; ZooBot provides a short-lived observer token URL that serves a local bootstrap page and keeps the password in URL fragment (instead of URL query).
...
- Image missing: build with [`scripts/sandbox-setup.sh`](https://github.com/zoo-bot/zoo-bot/blob/main/scripts/sandbox-setup.sh) or set `agents.defaults.sandbox.docker.image`.
- Container not running: it will auto-create per session on demand.
- Permission errors in sandbox: set `docker.user` to a UID:GID that matches your
  mounted workspace ownership (or chown the workspace folder).
- Custom tools not found: ZooBot runs commands with `sh -lc` (login shell), which
