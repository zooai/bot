...
When `BOT_NIX_MODE=1` is set (automatic with nix-openclaw):
...
OpenClaw reads JSON5 config from `BOT_CONFIG_PATH` and stores mutable data in `BOT_STATE_DIR`.
...
- `BOT_CONFIG_PATH` (default: `$BOT_STATE_DIR/openclaw.json`)
