...
Use SDK subpaths instead of the monolithic `zoo-bot/plugin-sdk` import when
authoring plugins:

- `zoo-bot/plugin-sdk/core` for generic plugin APIs, provider auth types, and shared helpers.
- `zoo-bot/plugin-sdk/compat` for bundled/internal plugin code that needs broader shared runtime helpers than `core`.
- `zoo-bot/plugin-sdk/telegram` for Telegram channel plugins.
- `zoo-bot/plugin-sdk/discord` for Discord channel plugins.
- `zoo-bot/plugin-sdk/slack` for Slack channel plugins.
- `zoo-bot/plugin-sdk/signal` for Signal channel plugins.
- `zoo-bot/plugin-sdk/imessage` for iMessage channel plugins.
- `zoo-bot/plugin-sdk/whatsapp` for WhatsApp channel plugins.
- `zoo-bot/plugin-sdk/line` for LINE channel plugins.
- `zoo-bot/plugin-sdk/msteams` for the bundled Microsoft Teams plugin surface.
- Bundled extension-specific subpaths are also available:
  `zoo-bot/plugin-sdk/acpx`, `zoo-bot/plugin-sdk/bluebubbles`,
  `zoo-bot/plugin-sdk/copilot-proxy`, `zoo-bot/plugin-sdk/device-pair`,
  `zoo-bot/plugin-sdk/diagnostics-otel`, `zoo-bot/plugin-sdk/diffs`,
  `zoo-bot/plugin-sdk/feishu`,
  `zoo-bot/plugin-sdk/google-gemini-cli-auth`, `zoo-bot/plugin-sdk/googlechat`,
  `zoo-bot/plugin-sdk/irc`, `zoo-bot/plugin-sdk/llm-task`,
  `zoo-bot/plugin-sdk/lobster`, `zoo-bot/plugin-sdk/matrix`,
  `zoo-bot/plugin-sdk/mattermost`, `zoo-bot/plugin-sdk/memory-core`,
  `zoo-bot/plugin-sdk/memory-lancedb`,
  `zoo-bot/plugin-sdk/minimax-portal-auth`,
  `zoo-bot/plugin-sdk/nextcloud-talk`, `zoo-bot/plugin-sdk/nostr`,
  `zoo-bot/plugin-sdk/open-prose`, `zoo-bot/plugin-sdk/phone-control`,
  `zoo-bot/plugin-sdk/qwen-portal-auth`, `zoo-bot/plugin-sdk/synology-chat`,
  `zoo-bot/plugin-sdk/talk-voice`, `zoo-bot/plugin-sdk/test-utils`,
  `zoo-bot/plugin-sdk/thread-ownership`, `zoo-bot/plugin-sdk/tlon`,
  `zoo-bot/plugin-sdk/twitch`, `zoo-bot/plugin-sdk/voice-call`,
  `zoo-bot/plugin-sdk/zalo`, and `zoo-bot/plugin-sdk/zalouser`.

Compatibility note:

- `zoo-bot/plugin-sdk` remains supported for existing external plugins.
...
- Read-only command paths such as `zoo-bot status`, `zoo-bot status --all`,
  `zoo-bot channels status`, `zoo-bot channels resolve`, and doctor/config
...
- `<workspace>/.zoo-bot/extensions/*.ts`
- `<workspace>/.zoo-bot/extensions/*/index.ts`
...
`plugins.entries.<id>.enabled` or `zoo-bot plugins enable <id>`.
...
Each plugin must include a `zoo-bot.plugin.json` file in its root. If a path
...
A plugin directory may include a `package.json` with `zoo-bot.extensions`:
...
Security guardrail: every `zoo-bot.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.

Security note: `zoo-bot plugins install` installs plugin dependencies with
`npm install --ignore-scripts` (no lifecycle scripts). Keep plugin dependency
trees "pure JS/TS" and avoid packages that require `postinstall` builds.

### Channel catalog metadata

Channel plugins can advertise onboarding metadata via `zoo-bot.channel` and
install hints via `zoo-bot.install`. This keeps the core catalog data-free.
...
- `~/.zoo-bot/mpm/catalog.json`
- `~/.zoo-bot/plugins/catalog.json`

Or point `BOT_PLUGIN_CATALOG_PATHS` (or `BOT_MPM_CATALOG_PATHS`) at
one or more JSON files (comma/semicolon/`PATH`-delimited). Each file should
contain `{ "entries": [ { "name": "@scope/pkg", "zoo-bot": { "channel": {...}, "install": {...} } } ] }`.
...
- Plugin config is validated using the JSON Schema embedded in
  `zoo-bot.plugin.json` (`configSchema`).
...
zoo-bot plugins install <path>                 # copy a local file/dir into ~/.zoo-bot/extensions/<id>
zoo-bot plugins install ./extensions/voice-call # relative path ok
zoo-bot plugins install ./plugin.tgz           # install from a local tarball
zoo-bot plugins install ./plugin.zip           # install from a local zip
zoo-bot plugins install -l ./extensions/voice-call # link (no copy) for dev
zoo-bot plugins install @hanzo/bot-voice-call # install from npm
zoo-bot plugins install @hanzo/bot-voice-call --pin # store exact resolved name@version
...
- Plugin-managed hooks show up in `zoo-bot hooks list` with `plugin:<id>`.
- You cannot enable/disable plugin-managed hooks via `zoo-bot hooks`; enable/disable the plugin instead.
...
- `zoo-bot models auth login --provider <id> [--method <id>]`
...
- Main package: `zoo-bot` (this repo)
- Plugins: separate npm packages under `@hanzo/bot-*` (example: `@hanzo/bot-voice-call`)

Publishing contract:

- Plugin `package.json` must include `zoo-bot.extensions` with one or more entry files.
- Entry files can be `.js` or `.ts` (jiti loads TS at runtime).
- `zoo-bot plugins install <npm-spec>` uses `npm pack`, extracts into `~/.zoo-bot/extensions/<id>/`, and enables it in config.
...
- CLI: `zoo-bot voicecall start|status`
- Tool: `voice_call`
...
- Separately published plugins should run their own CI (lint/build/test) and validate `zoo-bot.extensions` points at the built entrypoint (`dist/index.js`).
