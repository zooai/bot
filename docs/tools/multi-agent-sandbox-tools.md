...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `zoo-bot sandbox explain`.
...
Legacy `agent.*` configs are migrated by `zoo-bot doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=zoo-bot-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${BOT_STATE_DIR:-$HOME/.zoo-bot}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
