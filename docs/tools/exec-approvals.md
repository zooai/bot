...
- **gateway host** → `zoo-bot` process on the gateway machine
...
- allowlist entries live in host-local `~/.zoo-bot/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `zoo-bot approvals allowlist ...`).
- `zoo-bot security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `zoo-bot doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
`~/.zoo-bot/exec-approvals.json` directly.

CLI: `zoo-bot approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
