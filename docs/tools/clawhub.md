...
`clawhub sync` scans your current workdir first. If no skills are found, it falls back to known legacy locations (for example `~/zoo-bot/skills` and `~/.zoo-bot/skills`). This is designed to find older skill installs without extra flags.
