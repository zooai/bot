...
`<workspace>/skills` (highest) → `~/.zoo-bot/skills` → bundled skills (lowest)

Additionally, you can configure extra skill folders (lowest precedence) via
`skills.load.extraDirs` in `~/.zoo-bot/zoo-bot.json`.
...
- **Shared skills** live in `~/.zoo-bot/skills` (managed/local) and are visible
...
`zoo-bot.plugin.json` (paths relative to the plugin root). Plugin skills load
when the plugin is enabled and participate in the normal skill precedence rules.
You can gate them via `metadata.zoo-bot.requires.config` on the plugin’s config
...
- Optional frontmatter keys:
  - `homepage` — URL surfaced as “Website” in the macOS Skills UI (also supported via `metadata.zoo-bot.homepage`).
...
- `requires.config` — list of `zoo-bot.json` paths that must be truthy.
...
- Download installs: `url` (required), `archive` (`tar.gz` | `tar.bz2` | `zip`), `extract` (default: auto when archive detected), `stripComponents`, `targetDir` (default: `~/.zoo-bot/tools/<skillKey>`).

If no `metadata.zoo-bot` is present, the skill is always eligible (unless
disabled in config or blocked by `skills.allowBundled` for bundled skills).

## Config overrides (`~/.zoo-bot/zoo-bot.json`)
...
`metadata.zoo-bot.skillKey`, use that key under `skills.entries`.

Rules:

- `enabled: false` disables the skill even if it’s bundled/installed.
- `env`: injected **only if** the variable isn’t already set in the process.
- `apiKey`: convenience for skills that declare `metadata.zoo-bot.primaryEnv`.
