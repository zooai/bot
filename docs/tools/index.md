...
You can globally allow/deny tools via `tools.allow` / `tools.deny` in `zoo-bot.json`
...
- `host=node` can target a macOS companion app or a headless node host (`zoo-bot node run`).
...
- Requires a Brave API key (recommended: `zoo-bot configure --section web`, or set `BRAVE_API_KEY`).
...
- Quick smoke: `zoo-bot nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `zoo-bot gateway` restart in-place)
...
- `sessions_spawn` supports inline file attachments for subagent runtime only (ACP rejects them). Each attachment has `name`, `content`, and optional `encoding` (`utf8` or `base64`) and `mimeType`. Files are materialized into the child workspace at `.zoo-bot/attachments/<uuid>/` with a `.manifest.json` metadata file. The tool returns a receipt with `count`, `totalBytes`, per file `sha256`, and `relDir`. Attachment content is automatically redacted from transcript persistence.
