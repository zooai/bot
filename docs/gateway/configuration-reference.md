...
description: "Complete field-by-field reference for ~/.zoo-bot/zoo-bot.json"
summary: "Complete reference for every ZooBot config key, defaults, and channel settings"
read_when:
...
Every field available in `~/.zoo-bot/zoo-bot.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — ZooBot uses safe defaults when omitted.
...
- Legacy single-account Baileys auth dir is migrated by `zoo-bot doctor` into `whatsapp/default`.
...
- In multi-account setups (2+ account ids), set an explicit default (`channels.telegram.defaultAccount` or `channels.telegram.accounts.default`) to avoid fallback routing; `zoo-bot doctor` warns when this is missing or invalid.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["zoo-bot-dm"] },
      guilds: {
        "123456789012345678": {
...
- ZooBot additionally attempts voice receive recovery by leaving/rejoining a voice session after repeated decrypt failures.
...
Mattermost ships as a plugin: `zoo-bot plugins install @hanzo/bot-mattermost`.
...
- `commands.callbackUrl` must resolve to the ZooBot gateway endpoint and be reachable from the Mattermost server.
...
ZooBot spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
- If you add a non-default account via `zoo-bot channels add` (or channel onboarding) while still on a single-account top-level channel config, ZooBot moves account-scoped top-level single-account values into `channels.<channel>.accounts.default` first so the original account keeps working.
- Existing channel-only bindings (no `accountId`) keep matching the default account; account-scoped bindings remain optional.
- `zoo-bot doctor --fix` also repairs mixed shapes by moving account-scoped top-level single-account values into `accounts.default` when named accounts exist but `default` is missing.
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@zoo-bot"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `zoo-bot.json`).
...
Optional repository root shown in the system prompt's Runtime line. If unset, ZooBot auto-detects by walking upward from the workspace.
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, ZooBot assumes `anthropic` (deprecated).
    ...
    defaults: {
      sandbox: {
        ...
        browser: {
          ...
          network: "zoo-bot-sandbox-browser",
          cdpPort: 9222,
...
- `none`: per-scope sandbox workspace under `~/.zoo-bot/sandboxes`
...
**Sandboxed browser** (`sandbox.browser.enabled`): Chromium + CDP in a container. noVNC URL injected into system prompt. Does not require `browser.enabled` in `zoo-bot.json`.
noVNC observer access uses VNC auth by default and ZooBot emits a short-lived token URL (instead of exposing the password in the shared URL).

- `allowHostControl: false` (default) blocks sandboxed sessions from targeting the host browser.
- `network` defaults to `zoo-bot-sandbox-browser` (dedicated bridge network). Set to `bridge` only when you explicitly want global bridge connectivity.
    ...
    list: [
      {
        ...
        agentDir: "~/.zoo-bot/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@zoo-bot"] },
        sandbox: { mode: "off" },
        runtime: {
          type: "acp",
          acp: {
...
For `type: "acp"` entries, ZooBot resolves by exact conversation identity (`match.channel` + account + `match.peer.id`) and does not use the route binding tier order above.
...
- **`parentForkMaxTokens`**: max parent-session `totalTokens` allowed when creating a forked thread session (default `100000`).
  - If parent `totalTokens` is above this value, ZooBot starts a fresh thread session instead of inheriting parent transcript history.
...
- When `openai.baseUrl` points to a non-OpenAI endpoint, ZooBot treats it as an OpenAI-compatible TTS server and relaxes model/voice validation.
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:zoo-bot`   | All built-in tools (excludes provider plugins)                                           |
...
- Files are materialized into the child workspace at `.zoo-bot/attachments/<uuid>/` with a `.manifest.json`.
...
ZooBot uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.zoo-bot/agents/<agentId>/agent/models.json`.
...
- `models.providers.*.models.*.compat.supportsDeveloperRole`: optional compatibility hint. For `api: "openai-completions"` with a non-empty non-native `baseUrl` (host not `api.openai.com`), ZooBot forces this to `false` at runtime. Empty/omitted `baseUrl` keeps default OpenAI behavior.
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `zoo-bot onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `zoo-bot onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `zoo-bot onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `zoo-bot onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `zoo-bot onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `zoo-bot onboard --auth-choice minimax-api`.
...
- Loaded from `~/.zoo-bot/extensions`, `<workspace>/.zoo-bot/extensions`, plus `plugins.load.paths`.
...
- `plugins.installs`: CLI-managed install metadata used by `zoo-bot plugins update`.
...
Convenience flags: `--dev` (uses `~/.zoo-bot-dev` + port `19001`), `--profile <name>` (uses `~/.zoo-bot-<name>`).
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.zoo-bot/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-zoo-bot-token: <token>`.
...
- Hostname defaults to `zoo-bot`. Override with `BOT_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.zoo-bot/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `zoo-bot dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.zoo-bot/.env` (neither overrides existing vars).
...
- `secrets apply` targets supported `zoo-bot.json` credential paths.
...
```json5
{
  secrets: {
    providers: {
      ...
      filemain: {
        source: "file",
        path: "~/.zoo-bot/secrets.json",
        mode: "json",
        timeoutMs: 5000,
      },
      vault: {
        source: "exec",
        command: "/usr/local/bin/zoo-bot-vault-resolver",
...
- Legacy OAuth imports from `~/.zoo-bot/credentials/oauth.json`.
...
- Default log file: `/tmp/zoo-bot/zoo-bot-YYYY-MM-DD.log`.
...
- `cli.banner.taglineMode` controls banner tagline style:
  ...
  - `"default"`: fixed neutral tagline (`All your chats, one ZooBot.`).
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `zoo-bot doctor --fix` can strip unknown keys).
...
- Paths: resolved relative to the including file, but must stay inside the top-level config directory (`dirname` of `zoo-bot.json`). Absolute/`../` forms are allowed only when they still resolve inside that boundary.
