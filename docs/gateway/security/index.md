...
> Bot is **not** a hostile multi-tenant security boundary for multiple adversarial users sharing one agent/gateway.
...
Bot security guidance assumes a **personal assistant** deployment: one trusted operator boundary, potentially many agents.
...
Bot assumes the host and config boundary are trusted:
...
- Bot can run multiple gateway instances on one machine, but recommended operations favor clean trust-boundary separation.
- Recommended default: one user per machine/host (or VPS), one gateway for that user, and one or more agents in that gateway.
- If multiple users want Bot, use one VPS/host per user.
...
Bot is designed as a personal assistant security model: one trusted operator boundary, potentially many agents.
...
| -------------------------------------------------- | ------------- | ------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------- | -------- |
| `fs.state_dir.perms_world_writable`                | critical      | Other users/processes can modify full Bot state                                      | filesystem perms on `~/.zoo-bot`                                                                  | yes      |
...
Complete `dangerous*` / `dangerously*` config keys defined in Bot config
schema:
...
- Bot gateway is local/loopback first. If you terminate TLS at a reverse proxy, set HSTS on the proxy-facing HTTPS domain there.
- If the gateway itself terminates HTTPS, you can set `gateway.http.securityHeaders.strictTransportSecurity` to emit the HSTS header from Bot responses.
...
Bot stores session transcripts on disk under `~/.zoo-bot/agents/<agentId>/sessions/*.jsonl`.
...
Bot includes explicit bypass flags that disable external-content safety wrapping:
...
If you run Bot with Docker on a VPS, remember that published container ports
...
UI/WebSocket authentication. Bot verifies the identity by resolving the
...
Bot’s browser network policy defaults to the trusted-operator model: private/internal destinations are allowed unless you explicitly disable them.
