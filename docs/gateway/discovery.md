...
The design goal is to keep all network discovery/advertising in the **Node Gateway** (`zoo-bot gateway`) and keep clients (mac app, iOS) as consumers.
...
- Service types:
  - `_zoo-bot-gw._tcp` (gateway transport beacon)
- TXT keys (non-secret):
  - `role=gateway`
  ...
  - `cliPath=<path>` (optional; absolute path to a runnable `zoo-bot` entrypoint or binary)
