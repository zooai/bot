...
<Tip>
**New to configuration?** Start with `zoo-bot onboard` for interactive setup, or check out the [Configuration Examples](/gateway/configuration-examples) guide for complete copy-paste configs.
</Tip>
...
<Tabs>
  <Tab title="Interactive wizard">
    ```bash
    zoo-bot onboard       # full setup wizard
    zoo-bot configure     # config wizard
    ```
  </Tab>
  ...
  <Tab title="Direct edit">
    Edit `~/.zoo-bot/zoo-bot.json` directly. The Gateway watches the file and applies changes automatically (see [hot reload](#config-hot-reload)).
  </Tab>
</Tabs>
...
- Only diagnostic commands work (`zoo-bot doctor`, `zoo-bot logs`, `zoo-bot health`, `zoo-bot status`)
- Run `zoo-bot doctor` to see exact issues
- Run `zoo-bot doctor --fix` (or `--yes`) to apply repairs
        ...
        list: [
          {
            id: "main",
            groupChat: {
              mentionPatterns: ["@zoo-bot", "zoo-bot"],
            },
          },
        ],
      },
...
The Gateway watches `~/.zoo-bot/zoo-bot.json` and applies changes automatically — no manual restart needed for most settings.
...
<AccordionGroup>
  <Accordion title="config.apply (full replace)">
    ...
    <Warning>
    `config.apply` replaces the **entire config**. Use `config.patch` for partial updates, or `zoo-bot config set` for single keys.
    </Warning>
    ...
    zoo-bot gateway call config.apply --params '{
      "raw": "{ agents: { defaults: { workspace: \"~/.zoo-bot/workspace\" } } }",
      "baseHash": "<hash>",
