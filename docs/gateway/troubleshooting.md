...
- `zoo-bot gateway status` shows `Runtime: running` and `RPC probe: ok`.
- `zoo-bot doctor` reports no blocking config/service issues.
- `zoo-bot channels status --probe` shows connected/ready channels.
...
zoo-bot config get agents.defaults.models
```
...
- `Gateway start blocked: set gateway.mode=local` → local gateway mode is not enabled. Fix: set `gateway.mode="local"` in your config (or run `zoo-bot configure`). If you are running ZooBot via Podman using the dedicated `zoo-bot` user, the config lives at `~zoo-bot/.zoo-bot/zoo-bot.json`.
...
zoo-bot approvals get --node <idOrNameOrIp>
zoo-bot logs --follow
zoo-bot status
```
...
zoo-bot browser start --browser-profile zoo-bot
...
```bash
zoo-bot gateway status
zoo-bot config get gateway.mode
zoo-bot config get gateway.remote.url
zoo-bot config get gateway.auth.mode
```
...
```bash
zoo-bot config get gateway.bind
zoo-bot config get gateway.auth.token
zoo-bot gateway status
zoo-bot logs --follow
```
