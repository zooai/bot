...
```json5
{
  agent: { workspace: "~/.zoo-bot/workspace" },
  channels: { whatsapp: { allowFrom: ["+15555550123"] } },
}
```

Save to `~/.zoo-bot/zoo-bot.json` and you can DM the bot from that number.
...
```json5
{
  ...
  session: {
    ...
    store: "~/.zoo-bot/agents/default/sessions/sessions.json",
    maintenance: {
      mode: "warn",
      pruneAfter: "30d",
      maxEntries: 500,
