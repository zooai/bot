...
Save this as `~/Library/LaunchAgents/ai.zoo.bot.ssh-tunnel.plist`:
...
<dict>
    <key>Label</key>
    <string>ai.zoo.bot.ssh-tunnel</string>
...
```bash
launchctl bootstrap gui/$UID ~/Library/LaunchAgents/ai.zoo.bot.ssh-tunnel.plist
```
...
Legacy note: remove any leftover `com.zoo-bot.ssh-tunnel` LaunchAgent if present.
...
```bash
launchctl kickstart -k gui/$UID/ai.zoo.bot.ssh-tunnel
```

**Stop the tunnel:**

```bash
launchctl bootout gui/$UID/ai.zoo.bot.ssh-tunnel
```
