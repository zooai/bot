...
2. Put it on the **gateway host** (the machine running `zoo-bot gateway`).
...
3. If the Gateway runs under systemd/launchd, prefer putting the key in
   `~/.zoo-bot/.env` so the daemon can read it:
...
API keys for daemon use: `zoo-bot onboard`.

See [Help](/help) for details on env inheritance (`env.shellEnv`,
`~/.zoo-bot/.env`, systemd/launchd).
...
Run `zoo-bot models status` to confirm which profile is expiring. If the profile
