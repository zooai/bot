...
If the page fails to load, start the Gateway first: `zoo-bot gateway`.
...
you revoke it with `zoo-bot devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.zoo-bot/zoo-bot.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop zoo-bot`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. ZooBot
...
```bash
zoo-bot gateway --bind tailnet --token "$(openssl rand -hex 32)"
```
...
ZooBot **blocks** Control UI connections without device identity.
...
```bash
BOT_CONTROL_UI_BASE_PATH=/zoo-bot/ pnpm ui:build
```
