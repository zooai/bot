...
```bash
zoo-bot tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `zoo-bot tui --deliver`
...
- Check the Gateway logs: `zoo-bot logs --follow`.
- Confirm the agent can run: `zoo-bot status` and `zoo-bot models status`.
...
- No agents in picker: check `zoo-bot agents list` and your routing config.
