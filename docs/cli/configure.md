---
summary: "CLI reference for `zoo-bot configure` (interactive configuration prompts)"
read_when:
...
Tip: `zoo-bot config` without a subcommand opens the same wizard. Use
`zoo-bot config get|set|unset` for non-interactive edits.
...
```bash
zoo-bot configure
zoo-bot configure --section model --section channels
```
