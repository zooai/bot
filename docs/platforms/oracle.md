...
```bash
# Set hostname
sudo hostnamectl set-hostname zoo-bot
...
sudo tailscale up --ssh --hostname=zoo-bot
```

This enables Tailscale SSH, so you can connect via `ssh zoo-bot` from any device on your tailnet — no public IP needed.
...
zoo-bot config set gateway.bind loopback

# Require auth for the Gateway + Control UI
zoo-bot config set gateway.auth.mode token
zoo-bot doctor --generate-gateway-token

# Expose over Tailscale Serve (HTTPS + tailnet access)
zoo-bot config set gateway.tailscale.mode serve
zoo-bot config set gateway.trustedProxies '["127.0.0.1"]'

systemctl --user restart zoo-bot-gateway
```
...
# Check daemon status
systemctl --user status zoo-bot-gateway
...
```
https://zoo-bot.<tailnet-name>.ts.net/
```
...
This setup often removes the _need_ for extra host-based firewall rules purely to stop Internet-wide SSH brute force — but you should still keep the OS updated, run `zoo-bot security audit`, and verify you aren’t accidentally listening on public interfaces.
...
- **Credential permissions:** `chmod 700 ~/.zoo-bot`
- **Security audit:** `zoo-bot security audit`
...
ssh -L 18789:127.0.0.1:18789 ubuntu@zoo-bot
```
...
# Re-authenticate
sudo tailscale up --ssh --hostname=zoo-bot --reset
```
...
journalctl --user -u zoo-bot-gateway -n 50
```
...
- `~/.zoo-bot/` — config, credentials, session data
- `~/.zoo-bot/workspace/` — workspace (SOUL.md, memory, artifacts)

Back up periodically:

```bash
tar -czvf zoo-bot-backup.tar.gz ~/.zoo-bot ~/.zoo-bot/workspace
```
