...
- You can run the CLI (`zoo-bot`) on the gateway machine (or via SSH).
...
- Set `gateway.bind: "tailnet"` in `~/.zoo-bot/zoo-bot.json` on the gateway host.
...
1. Set up a DNS-SD zone (example `zoo-bot.internal.`) on the gateway host and publish `_zoo-bot-gw._tcp` records.
...
1. Create `~/.zoo-bot/workspace/canvas/index.html` on the gateway host.

2. Navigate the node to it (LAN):

```bash
zoo-bot nodes invoke --node "<Android Node>" --command canvas.navigate --params '{"url":"http://<gateway-hostname>.local:18789/__bot__/canvas/"}'
```
