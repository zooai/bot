...
# Check service
systemctl --user status zoo-bot-gateway.service

# View logs
journalctl --user -u zoo-bot-gateway.service -f
```
...
```bash
zoo-bot channels login whatsapp
# Scan QR code
```
...
- `~/.zoo-bot/workspace/` — workspace (SOUL.md, memory, etc.)
...
journalctl -u zoo-bot --no-pager -n 50
```
