...
service via the external `zoo-bot` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.zoo.bot.gateway`
  (or `ai.zoo.bot.<profile>` when using `--profile`/`BOT_PROFILE`; legacy `com.zoo-bot.*` is supported).
