...
- `--dev`：将状态隔离到 `~/.zoo-bot-dev` 下并调整默认端口。
- `--profile <name>`：将状态隔离到 `~/.zoo-bot-<name>` 下。
- `--no-color`：禁用 ANSI 颜色。
- `--update`：`zoo-bot update` 的简写（仅限源码安装）。
- `-V`、`--version`、`-v`：打印版本并退出。
...
注意：插件可以添加额外的顶级命令（例如 `zoo-bot voicecall`）。

## 安全

- `zoo-bot security audit` — 审计配置 + 本地状态中常见的安全隐患。
- `zoo-bot security audit --deep` — 尽力进行实时 Gateway 网关探测。
- `zoo-bot security audit --fix` — 收紧安全默认值并 chmod 状态/配置。

## 插件

管理扩展及其配置：

- `zoo-bot plugins list` — 发现插件（使用 `--json` 获取机器可读输出）。
- `zoo-bot plugins info <id>` — 显示插件详情。
- `zoo-bot plugins install <path|.tgz|npm-spec>` — 安装插件（或将插件路径添加到 `plugins.load.paths`）。
- `zoo-bot plugins enable <id>` / `disable <id>` — 切换 `plugins.entries.<id>.enabled`。
- `zoo-bot plugins doctor` — 报告插件加载错误。
...
- `zoo-bot memory status` — 显示索引统计。
- `zoo-bot memory index` — 重新索引记忆文件。
- `zoo-bot memory search "<query>"` — 对记忆进行语义搜索。
...
- `--workspace <dir>`：智能体工作区路径（默认 `~/.zoo-bot/workspace`）。
- `--wizard`：运行新手引导向导。
...
非交互式配置辅助工具（get/set/unset）。不带子命令运行 `zoo-bot config` 会启动向导。
...
- `channels status`：检查 Gateway 网关可达性和渠道健康状况（`--probe` 运行额外检查；使用 `zoo-bot health` 或 `zoo-bot status --deep` 进行 Gateway 网关健康探测）。
- 提示：`channels status` 在检测到常见配置错误时会打印带有建议修复的警告（然后指向 `zoo-bot doctor`）。
...
- `zoo-bot status --usage`（打印完整的提供商明细）
- macOS 菜单栏（上下文下的用量部分）
...
`zoo-bot models` 是 `models status` 的别名。
...
`node` 运行**无头节点主机**或将其作为后台服务管理。参见 [`zoo-bot node`](/cli/node)。
...
浏览器控制 CLI（专用 Chrome/Brave/Edge/Chromium）。参见 [`zoo-bot browser`](/cli/browser) 和[浏览器工具](/tools/browser)。
