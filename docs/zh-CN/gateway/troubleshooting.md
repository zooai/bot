...
| `zoo-bot gateway probe`           | Gateway 网关发现 + 可达性（本地 + 远程目标）                                          | 当你怀疑正在探测错误的 Gateway 网关时 |
| `zoo-bot channels status --probe` | 向运行中的 Gateway 网关查询渠道状态（并可选探测）                                     | 当 Gateway 网关可达但渠道异常时       |
| `zoo-bot gateway status`          | 监管程序状态（launchd/systemd/schtasks）、运行时 PID/退出、最后的 Gateway 网关错误    | 当服务"看起来已加载"但没有运行时      |
| `zoo-bot logs --follow`           | 实时日志（运行时问题的最佳信号）                                                      | 当你需要实际的故障原因时              |

**分享输出：** 优先使用 `zoo-bot status --all`（它会隐藏令牌）。如果你粘贴 `zoo-bot status`，考虑先设置 `BOT_SHOW_SECRETS=0`（令牌预览）。
...
- 或直接设置：
  ```bash
  zoo-bot config set gateway.mode local
  ```
...
- 设置远程 URL 并保持 `gateway.mode=remote`：
  ```bash
  zoo-bot config set gateway.mode remote
  zoo-bot config set gateway.remote.url "wss://gateway.example.com"
  ```

**仅临时/开发使用：** 传递 `--allow-unconfigured` 以在没有
`gateway.mode=local` 的情况下启动 Gateway 网关。

**还没有配置文件？** 运行 `zoo-bot setup` 创建初始配置，然后重新运行
Gateway 网关。
...
变量如 `DISPLAY` 应该放在 `~/.zoo-bot/.env` 中（由 Gateway 网关早期加载）。
...
服务是用 Bun 或版本管理的 Node 路径安装的，运行 `zoo-bot doctor`
迁移到系统 Node 安装。
...
- 或将密钥烘焙到你的自定义沙箱镜像中
- 然后运行 `zoo-bot sandbox recreate --agent <id>`（或 `--all`）
...
- `gateway.mode` 必须为 `local` 才能运行 `zoo-bot gateway` 和服务。
- 如果你设置了 `gateway.mode=remote`，**CLI 默认**使用远程 URL。服务可能仍在本地运行，但你的 CLI 可能在探测错误的位置。使用 `zoo-bot gateway status` 查看服务解析的端口 + 探测目标（或传递 `--url`）。
- `zoo-bot gateway status` 和 `zoo-bot doctor` 在服务看起来正在运行但端口关闭时会显示日志中的**最后 Gateway 网关错误**。
...
**如果 `zoo-bot gateway status` 显示配置不匹配**

- `Config (cli): ...` 和 `Config (service): ...` 通常应该匹配。
- 如果不匹配，你几乎肯定是在编辑一个配置而服务运行的是另一个。
- 修复：从你希望服务使用的相同 `--profile` / `BOT_STATE_DIR` 重新运行 `zoo-bot gateway install --force`。

**如果 `zoo-bot gateway status` 报告服务配置问题**

- 监管程序配置（launchd/systemd/schtasks）缺少当前默认值。
- 修复：运行 `zoo-bot doctor` 更新它（或 `zoo-bot gateway install --force` 完全重写）。
...
**如果 `zoo-bot gateway status` 显示 `bind=tailnet` 但未找到 tailnet 接口**
...
症状：`pwd` 或文件工具显示 `~/.zoo-bot/sandboxes/...` 即使你
期望的是主机工作区。
...
- 如果你不确定哪些模型可用，运行 `zoo-bot models list` 或
  `zoo-bot models scan` 并选择一个支持的模型。
...
grep -n "agents\\|groupChat\\|mentionPatterns\\|channels\\.whatsapp\\.groups\\|channels\\.telegram\\.groups\\|channels\\.imessage\\.groups\\|channels\\.discord\\.guilds" \
  "${BOT_CONFIG_PATH:-$HOME/.zoo-bot/zoo-bot.json}"
```
...
zoo-bot logs --follow
# 或者如果你想快速过滤：
tail -f "$(ls -t /tmp/zoo-bot/zoo-bot-*.log | head -1)" | grep "blocked\\|skip\\|unauthorized"
```
...
```bash
zoo-bot logs --follow | grep "pairing request"
```
...
```bash
ls -la ~/.zoo-bot/agents/<agentId>/sessions/
```
...
# 查看最近的连接事件
zoo-bot logs --limit 200 | grep "connection\\|disconnect\\|logout"
```

**修复：** 通常在 Gateway 网关运行后会自动重连。如果卡住，重启 Gateway 网关进程（无论你如何监管它），或使用详细输出手动运行：

```bash
zoo-bot gateway --verbose
```

如果你已登出/取消关联：

```bash
zoo-bot channels logout
trash "${BOT_STATE_DIR:-$HOME/.zoo-bot}/credentials" # 如果 logout 无法完全清除所有内容
zoo-bot channels login --verbose       # 重新扫描二维码
```
...
```bash
grep "media\\|fetch\\|download" "$(ls -t /tmp/zoo-bot/zoo-bot-*.log | head -1)" | tail -20
```
...
- `zoo-bot doctor` 报告每个无效条目。
- `zoo-bot doctor --fix` 应用迁移/修复并重写配置。
- 诊断命令如 `zoo-bot logs`、`zoo-bot health`、`zoo-bot status`、`zoo-bot gateway status` 和 `zoo-bot gateway probe` 即使配置无效也能运行。

### "All models failed" — 我应该首先检查什么？

- **凭证**存在于正在尝试的提供商（认证配置文件 + 环境变量）。
- **模型路由**：确认 `agents.defaults.model.primary` 和回退是你可以访问的模型。
- `/tmp/zoo-bot/…` 中的 **Gateway 网关日志**以获取确切的提供商错误。
- **模型状态**：使用 `/model status`（聊天）或 `zoo-bot models status`（CLI）。
...
```bash
curl -fsSL https://zoo-bot.ai/install.sh | bash -s -- --install-method git --no-onboard
```
...
5. 运行 `zoo-bot channels status --probe` 获取审计提示。
...
如果重置不起作用，在 [`scripts/package-mac-app.sh`](https://github.com/zoo-bot/zoo-bot/blob/main/scripts/package-mac-app.sh) 中更改 `BUNDLE_ID`（例如，添加 `.test` 后缀）并重新构建。这会强制 macOS 将其视为新应用。
...
# 或：launchctl bootout gui/$UID/bot.molt.gateway（用 bot.molt.<profile> 替换；旧版 com.zoo-bot.* 仍然有效）
```
...
```bash
# 在配置中打开跟踪日志：
#   ${BOT_CONFIG_PATH:-$HOME/.zoo-bot/zoo-bot.json} -> { logging: { level: "trace" } }
#
# 然后运行详细命令将调试输出镜像到标准输出：
zoo-bot gateway --verbose
zoo-bot channels login --verbose
```
...
zoo-bot logs --follow
# 如果 RPC 宕机的备用方案
tail -20 /tmp/zoo-bot/zoo-bot-*.log
```
...
trash "${BOT_STATE_DIR:-$HOME/.zoo-bot}"
zoo-bot channels login         # 重新配对 WhatsApp
zoo-bot gateway restart           # 或：zoo-bot gateway
```
