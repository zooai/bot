...
2. 在专用区域下发布 `_zoo-bot-gw._tcp` 的 DNS‑SD 记录
   （示例：`zoo-bot.internal.`）。
...
- 从 `~/.zoo-bot/dns/<domain>.db` 提供你选择的域名服务（示例：`zoo-bot.internal.`）
...
你的发现域名中浏览 `_zoo-bot-gw._tcp`，无需多播。
...
- 在 `~/.zoo-bot/zoo-bot.json` 中设置 `gateway.bind: "tailnet"`。
- 重启 Gateway 网关（或重启 macOS 菜单栏应用）。

## 什么在广播

只有 Gateway 网关广播 `_zoo-bot-gw._tcp`。

## 服务类型

- `_zoo-bot-gw._tcp` — Gateway 网关传输信标（被 macOS/iOS/Android 节点使用）。
...
- `transport=gateway`
- `cliPath=<path>`（可选；可运行的 `zoo-bot` 入口点的绝对路径）
...
iOS 节点使用 `NWBrowser` 来发现 `_zoo-bot-gw._tcp`。
