...
- 当前工作目录中的 `.env`（如果存在）
- `~/.openclaw/.env`（即 `$BOT_STATE_DIR/.env`）作为全局回退 `.env`
...
- `~/.openclaw/credentials/oauth.json`（或 `$BOT_STATE_DIR/credentials/oauth.json`）
...
- 如果你希望 `models.json` 存储在其他位置，请使用 `BOT_AGENT_DIR`（或 `PI_CODING_AGENT_DIR`）覆盖智能体配置根目录（默认：`~/.openclaw/agents/main/agent`）。
...
Gateway 网关监视 `~/.openclaw/openclaw.json`（或 `BOT_CONFIG_PATH`）并自动应用更改。
...
- `~/.openclaw/openclaw.json`（或 `BOT_CONFIG_PATH`）
...
- `off`：完全禁用 mDNS 广播
- 主机名：默认为 `openclaw`（通告 `openclaw.local`）。通过 `BOT_MDNS_HOSTNAME` 覆盖。
