...
你可以在 `~/.zoo-bot/zoo-bot.json` 中覆盖此设置：
...
所有日志配置都在 `~/.zoo-bot/zoo-bot.json` 的 `logging` 下。
...
- 你也可以使用 `zoo-bot plugins enable diagnostics-otel` 启用插件。
...
- `zoo-bot.tokens`（计数器，属性：`zoo-bot.token`、`zoo-bot.channel`、`zoo-bot.provider`、`zoo-bot.model`）
- `zoo-bot.cost.usd`（计数器，属性：`zoo-bot.channel`、`zoo-bot.provider`、`zoo-bot.model`）
- `zoo-bot.run.duration_ms`（直方图，属性：`zoo-bot.channel`、`zoo-bot.provider`、`zoo-bot.model`）
- `zoo-bot.context.tokens`（直方图，属性：`zoo-bot.context`、`zoo-bot.channel`、`zoo-bot.provider`、`zoo-bot.model`）

消息流：

- `zoo-bot.webhook.received`（计数器，属性：`zoo-bot.channel`、`zoo-bot.webhook`）
- `zoo-bot.webhook.error`（计数器，属性：`zoo-bot.channel`、`zoo-bot.webhook`）
- `zoo-bot.webhook.duration_ms`（直方图，属性：`zoo-bot.channel`、`zoo-bot.webhook`）
- `zoo-bot.message.queued`（计数器，属性：`zoo-bot.channel`、`zoo-bot.source`）
- `zoo-bot.message.processed`（计数器，属性：`zoo-bot.channel`、`zoo-bot.outcome`）
- `zoo-bot.message.duration_ms`（直方图，属性：`zoo-bot.channel`、`zoo-bot.outcome`）

队列 + 会话：

- `zoo-bot.queue.lane.enqueue`（计数器，属性：`zoo-bot.lane`）
- `zoo-bot.queue.lane.dequeue`（计数器，属性：`zoo-bot.lane`）
- `zoo-bot.queue.depth`（直方图，属性：`zoo-bot.lane` 或 `zoo-bot.channel=heartbeat`）
- `zoo-bot.queue.wait_ms`（直方图，属性：`zoo-bot.lane`）
- `zoo-bot.session.state`（计数器，属性：`zoo-bot.state`、`zoo-bot.reason`）
- `zoo-bot.session.stuck`（计数器，属性：`zoo-bot.state`）
- `zoo-bot.session.stuck_age_ms`（直方图，属性：`zoo-bot.state`）
- `zoo-bot.run.attempt`（计数器，属性：`zoo-bot.attempt`）
...
- `zoo-bot.webhook.error`
  - `zoo-bot.channel`、`zoo-bot.webhook`、`zoo-bot.chatId`、`zoo-bot.error`
- `zoo-bot.message.processed`
  - `zoo-bot.channel`、`zoo-bot.outcome`、`zoo-bot.chatId`、`zoo-bot.messageId`、`zoo-bot.sessionKey`、`zoo-bot.sessionId`、`zoo-bot.reason`
- `zoo-bot.session.stuck`
  - `zoo-bot.state`、`zoo-bot.ageMs`、`zoo-bot.queueDepth`、`zoo-bot.sessionKey`、`zoo-bot.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `zoo-bot doctor`。
