...
### 第二层：Gateway 网关 + 开发智能体冒烟测试（"@zoo-bot"实际做的事）
...
- OpenRouter：`openrouter/...`（数百个模型；使用 `zoo-bot models scan` 查找支持工具+图像的候选模型）
...
- 如果实时测试说"无凭证"，用调试 `zoo-bot models list`/模型选择相同的方式调试。

- 配置文件存储：`~/.zoo-bot/credentials/`（首选；测试中"配置文件密钥"的含义）
...
- `BOT_CONFIG_DIR=...`（默认：`~/.zoo-bot`）挂载到 `/home/node/.zoo-bot`
- `BOT_WORKSPACE_DIR=...`（默认：`~/.zoo-bot/workspace`）挂载到 `/home/node/.zoo-bot/workspace`
