...
This is the full reference for the `zoo-bot onboard` CLI wizard.
...
<Steps>
  ...
  <Step title="Model/Auth">
    ...
    - OAuth credentials live in `~/.zoo-bot/credentials/oauth.json`; auth profiles live in `~/.zoo-bot/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth).
    - More detail: [/concepts/oauth](/concepts/oauth)
    <Note>
    Headless/server tip: complete OAuth on a machine with a browser, then copy
    `~/.zoo-bot/credentials/oauth.json` (or `$BOT_STATE_DIR/credentials/oauth.json`) to the
    gateway host.
    </Note>
  </Step>
  <Step title="Workspace">
    - Default `~/.zoo-bot/workspace` (configurable).
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via `zoo-bot pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts the Gateway (if needed) and runs `zoo-bot health`.
    - Tip: `zoo-bot status --deep` adds gateway health probes to status output (requires a reachable gateway).
  </Step>
...
- Stores it under `~/.zoo-bot/tools/signal-cli/<version>/`.
...
Typical fields in `~/.zoo-bot/zoo-bot.json`:
...
`zoo-bot agents add` writes `agents.list[]` and optional `bindings`.

WhatsApp credentials go under `~/.zoo-bot/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.zoo-bot/agents/<agentId>/sessions/`.
