...
For images, Bot downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from Bot session logs.
...
`cacheWrite`. If pricing is missing, Bot shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. Bot can
...
Anthropic's 1M context window is currently beta-gated. Bot can inject the
...
Bot skips the `context-1m-*` beta header because Anthropic currently
