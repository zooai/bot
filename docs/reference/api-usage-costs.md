...
- `zoo-bot status --usage` and `zoo-bot channels list` show provider **usage windows**
...
`zoo-bot models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
