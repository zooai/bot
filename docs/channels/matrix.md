...
```bash
zoo-bot plugins install @hanzo/bot-matrix
```
...
1. Install the Matrix plugin:
   - From npm: `zoo-bot plugins install @hanzo/bot-matrix`
   - From a local checkout: `zoo-bot plugins install ./extensions/matrix`
...
`~/.zoo-bot/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `zoo-bot pairing list matrix`
  - `zoo-bot pairing approve matrix <CODE>`
