...
```
    ...
    Telegram does **not** use `zoo-bot channels login telegram`; configure token in config/env, then start gateway.
...
<Tabs>
  <Tab title="DM policy">
    ...
    If you upgraded and your config contains `@username` allowlist entries, run `zoo-bot doctor --fix` to resolve them (best-effort; requires a Telegram bot token).
    If you previously relied on pairing-store allowlist files, `zoo-bot doctor --fix` can recover entries into `channels.telegram.allowFrom` in allowlist flows (for example when `dmPolicy: "allowlist"` has no explicit IDs yet).
...
```
    ...
    - or read `chat.id` from `zoo-bot logs --follow`
...
```
    ...
    - `~/.zoo-bot/telegram/sticker-cache.json`
...
```bash
zoo-bot message send --channel telegram --target 123456789 --message "hi"
zoo-bot message send --channel telegram --target @name --message "hi"
```

    Telegram polls use `zoo-bot message poll` and support forum topics:

```bash
zoo-bot message poll --channel telegram --target 123456789 \
  --poll-question "Ship it?" --poll-option "Yes" --poll-option "No"
zoo-bot message poll --channel telegram --target -1001234567890:topic:42 \
...
<AccordionGroup>
  <Accordion title="Bot does not respond to non mention group messages">
    ...
    - `zoo-bot channels status` warns when config expects unmentioned group messages.
    - `zoo-bot channels status --probe` can check explicit numeric group IDs; wildcard `"*"` cannot be membership-probed.
  ...
  <Accordion title="Bot not seeing group messages at all">
    ...
    - review logs: `zoo-bot logs --follow` for skip reasons
...
- `channels.telegram.allowFrom`: DM allowlist (numeric Telegram user IDs). `allowlist` requires at least one sender ID. `open` requires `"*"`. `zoo-bot doctor --fix` can resolve legacy `@username` entries to IDs and can recover allowlist entries from pairing-store files in allowlist migration flows.
