...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects ZooBot to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
zoo-bot plugins install @hanzo/bot-feishu
```

Local checkout (when running from a git repo):

```bash
zoo-bot plugins install ./extensions/feishu
```
...
If you just installed ZooBot, run the wizard:
...
2. Configuring app credentials in ZooBot
...
1. You already ran `zoo-bot channels add` for Feishu
2. The gateway is running (`zoo-bot gateway status`)
...
```bash
zoo-bot pairing approve feishu <CODE>
```
...
2. Run `zoo-bot logs --follow` and look for `chat_id`
...
2. Run `zoo-bot logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `zoo-bot gateway status`  | Show gateway status           |
| `zoo-bot gateway install` | Install/start gateway service |
| `zoo-bot gateway stop`    | Stop gateway service          |
| `zoo-bot gateway restart` | Restart gateway service       |
| `zoo-bot logs --follow`   | Tail gateway logs             |
...
4. Check logs: `zoo-bot logs --follow`
...
5. Ensure the gateway is running: `zoo-bot gateway status`
6. Check logs: `zoo-bot logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.zoo-bot/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.zoo-bot/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
