...
5. Configure `msteams` in `~/.zoo-bot/zoo-bot.json` (or env vars) and start the gateway.
...
1. **Install the Microsoft Teams plugin**
   - From npm: `zoo-bot plugins install @hanzo/bot-msteams`
   - From a local checkout: `zoo-bot plugins install ./extensions/msteams`
