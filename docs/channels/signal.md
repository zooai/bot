...
5. Pair your DM sender:
   ...
   - Approve code on the server: `zoo-bot pairing approve signal <PAIRING_CODE>`.
...
- Approve via:
  - `zoo-bot pairing list signal`
  - `zoo-bot pairing approve signal <CODE>`
...
- Config validation errors after edits: run `zoo-bot doctor --fix`.
...
zoo-bot pairing list signal
pgrep -af signal-cli
grep -i "signal" "/tmp/zoo-bot/zoo-bot-$(date +%Y-%m-%d).log" | tail -20
```
