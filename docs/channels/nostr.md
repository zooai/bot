...
read_when:
  - You want ZooBot to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables ZooBot to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`zoo-bot onboard`) and `zoo-bot channels add` list optional channel plugins.
...
```bash
zoo-bot plugins install @hanzo/bot-nostr
```

Use a local checkout (dev workflows):

```bash
zoo-bot plugins install --link <path-to-zoo-bot>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "zoo-bot@example.com",
        "lud16": "zoo-bot@example.com"
      }
    }
  }
}
