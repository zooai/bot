...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in ZooBot
...
Running ZooBot via Node with `tsx` fails at startup with:
...
- Before that (Bun path), `zoo-bot status` and `gateway:watch` worked.
...
- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node zoo-bot.mjs status` works on Node 25.
