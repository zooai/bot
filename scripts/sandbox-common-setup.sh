...
BASE_IMAGE="${BASE_IMAGE:-zoo-bot-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-zoo-bot-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=zoo-bot.sandbox=1)
NOTE
