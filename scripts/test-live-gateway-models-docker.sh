...
IMAGE_NAME="${BOT_IMAGE:-${CLAWDBOT_IMAGE:-zoo-bot:local}}"
CONFIG_DIR="${BOT_CONFIG_DIR:-${CLAWDBOT_CONFIG_DIR:-$HOME/.zoo-bot}}"
WORKSPACE_DIR="${BOT_WORKSPACE_DIR:-${CLAWDBOT_WORKSPACE_DIR:-$HOME/.zoo-bot/workspace}}"
...
docker run --rm -t \
  ...
  -v "$CONFIG_DIR":/home/node/.zoo-bot \
  -v "$WORKSPACE_DIR":/home/node/.zoo-bot/workspace \
  "${PROFILE_MOUNT[@]}" \
  "$IMAGE_NAME" \
