#!/usr/bin/env bash
# ClawDock - Docker helpers for ZooBot
# Inspired by Simon Willison's "Running ZooBot in Docker"
# https://til.simonwillison.net/llms/zoo-bot-docker
#
# Installation:
...
# Common paths to check for ZooBot
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/zoo-bot"
  "${HOME}/workspace/zoo-bot"
  "${HOME}/projects/zoo-bot"
  "${HOME}/dev/zoo-bot"
  "${HOME}/code/zoo-bot"
  "${HOME}/src/zoo-bot"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found ZooBot at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/zoo-bot"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ ZooBot not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/zoo-bot/zoo-bot.git ~/zoo-bot"
    echo "  cd ~/zoo-bot && ./docker-setup.sh"
    echo ""
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d zoo-bot-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart zoo-bot-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f zoo-bot-gateway
}
...
# Container Access
clawdock-shell() {
  _clawdock_compose exec zoo-bot-gateway \
    bash -c 'echo "alias zoo-bot=\"./zoo-bot.mjs\"" > /tmp/.bashrc_zoo-bot && bash --rcfile /tmp/.bashrc_zoo-bot'
}
...
clawdock-cli() {
  _clawdock_compose run --rm zoo-bot-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build zoo-bot-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "BOT_GATEWAY_TOKEN=$token" zoo-bot-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" zoo-bot-gateway \
    bash -c './zoo-bot.mjs config set gateway.remote.token "$TOKEN" && ./zoo-bot.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec zoo-bot-gateway \
    bash -c "./zoo-bot.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart zoo-bot-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm zoo-bot-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec zoo-bot-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'zoo-bot config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for ZooBot${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (zoo-bot alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to zoo-bot project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.zoo-bot)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'zoo-bot channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'zoo-bot status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.zoo-bot.ai${_CLR_RESET}"
  echo ""
}
