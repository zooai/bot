...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/zoo-bot/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "ZooBot Re-Auth" -c "After getting key, run: ssh $SERVER '~/zoo-bot/scripts/mobile-reauth.sh'" --id zoo-bot-auth
        ;;
    *)
