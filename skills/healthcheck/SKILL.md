---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Bot deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Bot cron scheduling for periodic checks, or version status checks on a machine running Bot (laptop, workstation, Pi, VPS).
---
...
Assess and harden the host running Bot, then align it to a user-defined risk tolerance without breaking access. Use Bot security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Bot changes the host firewall, SSH, or OS updates; it does not.
...
5. Bot gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Bot can access sensitive data.
...
### 2) Run Bot security audits (read-only)
...
Offer to apply Bot safe defaults (numbered):

1. `zoo-bot security audit --fix`

Be explicit that `--fix` only tightens Bot defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Bot version/update status (read-only)
...
- Credential hygiene notes (location of Bot creds, prefer disk encryption)
...
After Bot install or first hardening pass, run at least one baseline audit and version check:

- `zoo-bot security audit`
- `zoo-bot security audit --deep`
- `zoo-bot update status`

Ongoing monitoring is recommended. Use the Bot cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
Do not invent CLI flags or imply Bot enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Bot compaction:
