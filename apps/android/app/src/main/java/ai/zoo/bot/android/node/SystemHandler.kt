...
private const val NOTIFICATION_CHANNEL_BASE_ID = "zoo-bot.system.notify"
  ...
  private fun ensureChannel(priority: String?): String {
    ...
    val (suffix, importance, name) =
      when (normalizedPriority) {
        "passive" -> Triple("passive", NotificationManager.IMPORTANCE_LOW, "ZooBot Passive")
        "timesensitive" -> Triple("timesensitive", NotificationManager.IMPORTANCE_HIGH, "ZooBot Time Sensitive")
        else -> Triple("active", NotificationManager.IMPORTANCE_DEFAULT, "ZooBot Active")
      }
