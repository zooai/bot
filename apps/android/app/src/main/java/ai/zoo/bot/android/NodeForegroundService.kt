...
class NodeForegroundService : Service() {
  ...
  override fun onCreate() {
    super.onCreate()
    ensureChannel()
    val initial = buildNotification(title = "ZooBot Node", text = "Starting…")
    ...
    notificationJob =
      scope.launch {
        combine(
          ...
          val title = if (connected) "ZooBot Node · Connected" else "ZooBot Node"
          val micSuffix =
            if (micEnabled) {
  ...
  private fun ensureChannel() {
    ...
    val channel =
      NotificationChannel(
        ...
        description = "ZooBot node connection status"
        setShowBadge(false)
      }
  ...
  companion object {
    ...
    private const val ACTION_STOP = "ai.zoo.bot.android.action.STOP"
