...
actor RemoteTunnelManager {
    ...
    private let logger = Logger(subsystem: "ai.hanzo.bot", category: "remote-tunnel")
    ...
    func controlTunnelPortIfRunning() async -> UInt16? {
        ...
        // If a previous HanzoBot run already has an SSH listener on the expected port (common after restarts),
