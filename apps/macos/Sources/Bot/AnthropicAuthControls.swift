...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: BotOAuthStore.AnthropicOAuthStatus = BotOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: BotOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = BotOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = BotOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. HanzoBot can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: BotOAuthStore.AnthropicOAuthStatus,
