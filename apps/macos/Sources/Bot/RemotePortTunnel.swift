...
final class RemotePortTunnel {
    private static let logger = Logger(subsystem: "ai.hanzo.bot", category: "remote.tunnel")
    ...
    private static func resolveRemotePortOverride(for sshHost: String) -> Int? {
        ...
        let sshKey = BotConfigFile.hostKey(sshHost)
        let urlKey = BotConfigFile.hostKey(host)
