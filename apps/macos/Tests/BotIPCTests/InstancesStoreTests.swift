...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: HanzoBotProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: HanzoBotProtocol.AnyCodable] = [
            "presence": .init([HanzoBotProtocol.AnyCodable(entry)]),
        ]
        let payload = HanzoBotProtocol.AnyCodable(payloadMap)
