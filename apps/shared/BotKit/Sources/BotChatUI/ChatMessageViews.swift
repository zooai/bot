...
struct ChatMessageBubble: View {
    let message: HanzoBotChatMessage
    let style: HanzoBotChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? HanzoBotChatTheme.userText : HanzoBotChatTheme.assistantText
    ...
    private var inlineAttachments: [HanzoBotChatMessageContent] {
    ...
    private var toolCalls: [HanzoBotChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [HanzoBotChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? HanzoBotChatTheme.userBubble
        }
        if self.style == .onboarding {
            return HanzoBotChatTheme.onboardingAssistantBubble
        }
        return HanzoBotChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return HanzoBotChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: HanzoBotChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? HanzoBotChatTheme.userText : HanzoBotChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: HanzoBotChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(HanzoBotChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(HanzoBotChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [HanzoBotChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: HanzoBotChatTheme.assistantText)
            }
        }
    }
}
