...
@MainActor
struct HanzoBotChatComposer: View {
    @Bindable var viewModel: HanzoBotChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(HanzoBotChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(HanzoBotChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \HanzoBotPendingAttachment.id)
                { (att: HanzoBotPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            HanzoBotPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Rectangle()
                .fill(HanzoBotChatTheme.divider)
                .frame(height: 1)
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(HanzoBotChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(HanzoBotChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(HanzoBotChatTheme.subtleCard)
        .clipShape(Capsule())
    }
