...
export function resolveCanonicalConfigPath(
  ...
  const override = env.BOT_CONFIG_PATH?.trim() || env.BOT_CONFIG_PATH?.trim() || env.CLAWDBOT_CONFIG_PATH?.trim();
  if (override) {
...
export function resolveDefaultConfigCandidates(
  ...
  const effectiveHomedir = () => resolveRequiredHomeDir(env, homedir);
  const explicit = env.BOT_CONFIG_PATH?.trim() || env.BOT_CONFIG_PATH?.trim() || env.CLAWDBOT_CONFIG_PATH?.trim();
  if (explicit) {
