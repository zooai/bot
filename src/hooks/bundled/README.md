...
This directory contains hooks that ship with Zoo Bot. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
homepage: https://zoo.ngo/docs/automation/hooks#my-hook
metadata:
...
Hooks can be configured in `~/.zooai/bot.json`:
...
2. Restart gateway: `pkill -9 -f 'zoo-bot.*gateway' && pnpm zoo-bot gateway`
3. Enable the hook: `zoo-bot hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://zoo.ngo/docs/automation/hooks
