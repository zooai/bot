...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  ...
  records.push(`_zoo-bot-gw._tcp IN PTR ${instanceLabel}._zoo-bot-gw._tcp`);
  records.push(`${instanceLabel}._zoo-bot-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._zoo-bot-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
