...
describe("createZooBotCodingTools message provider policy", () => {
  it.each(["voice", "VOICE", " Voice "])(
    "does not expose tts tool for normalized voice provider: %s",
    (messageProvider) => {
      const tools = createZooBotCodingTools({ messageProvider });
  ...
  it("keeps tts tool for non-voice providers", () => {
    const tools = createZooBotCodingTools({ messageProvider: "discord" });
    const names = new Set(tools.map((tool) => tool.name));
