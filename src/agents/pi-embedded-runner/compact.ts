...
import { resolveZooBotDocsPath } from "../docs-path.js";
...
export async function compactEmbeddedPiSessionDirect(
  ...
  const agentDir = params.agentDir ?? resolveZooBotAgentDir();
  await ensureZooBotModelsJson(params.config, agentDir);
  ...
  try {
    ...
    const toolsRaw = createZooBotCodingTools({
      exec: {
    ...
    const promptMode =
      isSubagentSessionKey(params.sessionKey) || isCronSessionKey(params.sessionKey)
        ? "minimal"
        : "full";
    const docsPath = await resolveZooBotDocsPath({
      workspaceDir: effectiveWorkspace,
