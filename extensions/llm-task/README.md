...
`zoo-bot.invoke --each`) without adding new Zoo Bot code per workflow.
...
This extension depends on Zoo Bot internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Zoo Bot extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.zoo-bot/extensions` as a standalone plugin directory.
