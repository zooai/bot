...
This document describes how the ZooBot ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`zoo-bot acp` exposes an ACP agent over stdio and forwards prompts to a running
ZooBot Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
- ACP client spawns `zoo-bot acp` and speaks ACP messages over stdio.
...
`zoo-bot acp` resolves the Gateway URL and auth from CLI flags or config:
